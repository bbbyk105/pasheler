      ...
      <main className="py-16 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <h1 className="text-3xl font-serif text-stone-800 mb-4">{t.orderConfirmation.thankYou}</h1>
            <p className="text-lg text-stone-600 mb-2">
              {t.orderConfirmation.orderNumber}: <span className="font-medium">{orderData.orderNumber}</span>
            </p>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div className="bg-stone-50 p-8 rounded-lg">
              <h2 className="text-xl font-serif text-stone-800 mb-6">{t.orderConfirmation.orderSummary}</h2>
              
              {/* Items */}
              <div className="space-y-4 mb-6">
                {orderData.items.map((item: any) => (
                  <div key={`${item.id}-${item.variantId}`} className="flex gap-4 pb-4 border-b border-stone-200 last:border-b-0">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-stone-600 text-sm">{t.common.quantity}: {item.quantity}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-medium text-stone-800 text-sm">
                        {formatPrice(item.price * item.quantity, orderData.currency)}
                      </p>
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-3 pt-4 border-t border-stone-200">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-stone-800">{formatPrice(orderData.subtotal, orderData.currency)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-stone-600">{t.common.shipping}</span>
                  <span className="text-stone-800">
                    {orderData.shipping === 0 ? t.common.free : formatPrice(orderData.shipping, orderData.currency)}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-stone-600">{t.common.tax}</span>
                  <span className="text-stone-800">{formatPrice(orderData.tax, orderData.currency)}</span>
                </div>
                <div className="flex justify-between font-medium pt-3 border-t border-stone-200 text-lg">
                  ...
                  <span className="text-stone-800">{formatPrice(orderData.total, orderData.currency)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-8">
              <div className="bg-stone-50 p-8 rounded-lg">
                <h2 className="text-xl font-serif text-stone-800 mb-6">
                  {orderData.deliveryMethod === 'delivery' ? t.orderConfirmation.deliveryInfo : t.orderConfirmation.pickupInfo}
                </h2>
                ...
                {orderData.deliveryMethod === 'delivery' ? (
                  <div className="space-y-3 text-sm">
                    ...
                    <p className="text-stone-600">{orderData.customerInfo.address}</p>
                    <p className="text-stone-600">
                      {orderData.customerInfo.city}, {orderData.customerInfo.state} {orderData.customerInfo.zipCode}
                    </p>
                    <p className="text-stone-600">{orderData.customerInfo.country}</p>
