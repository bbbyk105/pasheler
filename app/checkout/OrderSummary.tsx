...
export default function OrderSummary({ deliveryMethod }: OrderSummaryProps) {
  ...
  return (
    <div className="bg-stone-50 p-6 rounded-lg">
      <h2 className="text-xl font-serif text-stone-800 mb-6">{t.checkout.orderSummary}</h2>
      ...
      <div className="space-y-3 pt-4 border-t border-stone-200">
        <div className="flex justify-between text-sm">
          ...
          <span className="text-stone-800">{formatPrice(subtotal, currency)}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-stone-600">{t.common.shipping}</span>
          <span className="text-stone-800">
            {shippingFee === 0 ? t.common.free : formatPrice(shippingFee, currency)}
          </span>
        </div>
        <div className="flex justify-between text-sm">
      ...
      <div className="mt-6 p-4 bg-green-50 rounded-lg">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-green-700">{t.checkout.secureCheckout}</span>
        </div>
      </div>
    </div>
  );
